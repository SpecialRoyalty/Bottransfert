...
async def upsert_group(session: AsyncSession, chat: Chat) -> GroupConfig:
    existing = await session.get(GroupConfig, chat.id)
...
async def list_groups(session: AsyncSession) -> list[GroupConfig]:
    result = await session.execute(
        select(GroupConfig).where(GroupConfig.is_active == True).order_by(GroupConfig.title)
    )
...
async def list_sources(session: AsyncSession) -> list[GroupConfig]:
    result = await session.execute(
        select(GroupConfig).where(GroupConfig.role == "source", GroupConfig.is_active == True).order_by(GroupConfig.title)
    )
    return list(result.scalars().all())
...
async def get_target(session: AsyncSession) -> GroupConfig | None:
    result = await session.execute(
        select(GroupConfig).where(GroupConfig.role == "target", GroupConfig.is_active == True).limit(1)
    )
